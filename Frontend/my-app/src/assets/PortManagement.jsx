...
const PortManagement = () => {
  ...
  if (isLoggedIn && isSystemAdmin) {
    return (
      <div>
        <div className="bg-cover bg-center bg-no-repeat h-[60vh] flex items-center justify-center my-1" style={{ backgroundImage: `url(${imageUrl})` }} >
          <div className="flex flex-col md:flex-row justify-center items-center gap-6 lg:gap-12 mt-4 w-full ">
            <button
              ...
              onClick={() => navigate("/addPort")}
            >
              Add Port
            </button>
            <button
              className="bg-white  text-base py-3 px-1 mx-6 text-cyan-700 w-2/5 md:w-1/4 lg:w-1/5 rounded-full shadow-lg hover:bg-gray-200 hover:text-cyan-900 transition duration-300 ease-in-out transform hover:scale-105 "
              onClick={() => navigate("/updatePort")}
            >
              Update Port
            </button>
          </div>
