...
const OrderDetails = () => {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            try {
                const responce = await fetch(`/api/order/${orderId.orderId}`)
                if (responce.ok) {
                    const orderData = await responce.json()
                    console.log(orderData.order)
                    const now = Date.now();
                    orderData.trackingHistory = null

                    const latestHistoryEntry = null

                    orderData.updatedAt = latestHistoryEntry
                        ? latestHistoryEntry.timestamp
                        : new Date(now).toISOString();

                    setOrderToRender(orderData.order);
                    setLoading(false);
                }
            }
            catch (error) {
